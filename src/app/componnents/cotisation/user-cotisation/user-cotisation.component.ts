...
@Component({
  selector: 'app-user-cotisation',
  standalone: true,
  imports: [PaginationComponent, CommonModule, FormsModule, ReactiveFormsModule, RouterModule],
  templateUrl: './user-cotisation.component.html',
  styleUrl: './user-cotisation.component.css'
})
export class UserCotisationComponent implements OnInit{
  ...
  constructor(private contisationsService: CotisationService,private fb: FormBuilder, private excelService: ExcelService,
     private router: Router, private cotisationService: CotisationService,
  ...
  ngOnInit(): void {
    this.contisationsService.getUserCotisations().subscribe(
      (response) => {
        if (response.success) {
  ...
  getmemberMatricule(countryId: number){
    ...
    payer(cotisation: any): void {
      const payment: Payment = {
        ...
        amount: cotisation.amount, // Assurez-vous que l'objet `item` contient cette information
